...
const getCategorizedCommands = (cmdPath) => {
  const categories = {};
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(configPath)) {
          ...
          if (config && config.emoji) categoryEmoji = `${config.emoji}`;
        }
      } catch (err) {}
...
const createCategoryEmbed = (categories, searchQuery = null) => {
  ...
  const categoryList = keys
    .map((category) => {
      ...
      return `${emoji} **${capitalizeFirstLetter(category)}** (${count} command${count !== 1 ? "s" : ""})`;
    })
    .join("\n");
...
const createCommandEmbed = (page, currentPage, totalPages, category, categories, searchQuery = null) => {
  ...
  const title = searchQuery
    ...
    : `${emoji} ${capitalizeFirstLetter(category)} Commands`;
...
const createCategorySelector = (categories, currentCategory = null) => {
  ...
  for (const category of keys.slice(0, MAX_SELECT_OPTIONS - 1)) {
    const labelRaw = `${categories[category].emoji ?? "📁"} ${capitalizeFirstLetter(category)}`;
...
const handleHelp = async (client, interaction) => {
  const commandQuery = interaction.options?.getString?.("command") ?? null;
  try {
    await interaction.deferReply();
    const categories = getCategorizedCommands(path.join(__dirname, ".."));
    ...
    if (commandQuery) {
      ...
      if (Object.keys(filteredCategories).length === 0) {
        return await interaction.editReply(`No commands found matching "${commandQuery}".`);
      }
    }
    ...
    collector.on("collect", async (selectInteraction) => {
      ...
      if (selectedValue === "more_categories") {
        ...
        const list = keys
          .map((k) => `${filteredCategories[k].emoji ?? "📁"} ${capitalizeFirstLetter(k)} (${filteredCategories[k].commands.length})`)
          .join("\n");
    ...
    console.log(error);
    try {
      await interaction.editReply("An error occurred while processing the help command.");
    } catch (editError) {
