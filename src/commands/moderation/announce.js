...
const handleAnnounce = async(client, interaction) => {
  ...
  try {
    ...
    interaction.editReply({ content: `Successfully sent an announcement to channel ${channel}`, flags: MessageFlags.Ephemeral })
  } catch (error) {
    console.log(error)
    announceEmbed.setDescription("An error occurred whilst sending the announcement.").setColor("Red")
