const { ApplicationCommandOptionType, PermissionFlagsBits, Client, Interaction, MessageFlags, Message } = require("discord.js");
...
const handleTimeout = async(client, interaction)=>{
  ...
  if(!targetUser){
    return interaction.editReply({
      content: "❌ Error: The user you mentioned is not available. They might have left the server or their account cannot be reached.",
  ...
  if(isNaN(msDuration)){
    return interaction.editReply({
      content: "❌ I can't use that duration. Please enter a valid timeout duration.",
      flags: MessageFlags.Ephemeral
    })
  }

  if(msDuration < 5000 || msDuration > 2.419e9){ // check if duration is less than 5 seconds and greater than 28 days
    return interaction.editReply({
      content: "❌ Invalid timeout duration. Please use a duration between 5 seconds and 28 days."
    })
  }
  ...
  try {
    ...
    if(targetUser.isCommunicationDisabled()){
      ...
      return interaction.editReply(`✅ **${targetUser}**'s timeout has been updated to **${prettyMs(msDuration, { verbose: true })}**.\n\n**Reason:** ${reason}`)
    }
...
module.exports = {
  ...
  permissionsRequired: [PermissionFlagsBits.Administrator, PermissionFlagsBits.MuteMembers],
  botPermissionsRequired: [PermissionFlagsBits.Administrator, PermissionFlagsBits.MuteMembers],
